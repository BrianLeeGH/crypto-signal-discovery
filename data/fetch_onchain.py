...
from config.settings import DEFAULT_DAYS, FEAR_GREED_API, RAW_DATA_DIR
...
def fetch_blockchain_info_data(chart_type="n-unique-addresses", days=180):
    """
    从 blockchain.info 获取基础链上数据
    chart_type:
        - 'n-unique-addresses': 活跃地址数
        - 'hash-rate': 哈希率
        - 'market-price': 价格 (对比用)
        - 'mvrv': MVRV Ratio (如果可用)
    """
    print(f"Fetching {chart_type} from blockchain.info...")
    try:
        url = f"https://api.blockchain.info/charts/{chart_type}?timespan={days}days&format=json&cors=true"
        response = requests.get(url, timeout=30)
        data = response.json()
        ...
        df = df.rename(columns={"y": chart_type})
        df = df.set_index("timestamp")

        print(f"  Done. {len(df)} records for {chart_type}")
...
def main():
    ...
    # 1. Fear & Greed Index - 免费可用
    fng = fetch_fear_greed_index(DEFAULT_DAYS)
    save_data(fng, "fear_greed_index.csv")

    print("\n" + "=" * 50 + "\n")

    # 2. Blockchain.info 链上数据 (BTC)
    active_addresses = fetch_blockchain_info_data("n-unique-addresses", DEFAULT_DAYS)
    save_data(active_addresses, "btc_active_addresses.csv")

    hash_rate = fetch_blockchain_info_data("hash-rate", DEFAULT_DAYS)
    save_data(hash_rate, "btc_hash_rate.csv")

    print("\n" + "=" * 50 + "\n")

    # 3. Coinglass 数据 - 可能有限制
